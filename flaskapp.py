...
def index():
    if "rid" in session:
        os.remove(
            os.path.join(app.root_path, "receipts", "booking%s.txt" % session["rid"])
        )
        session.pop("rid")
    conn = dbfunc.getConnection()  # connection to DB
    if conn != None:  # Checking if connection is None
        if conn.is_connected():
            ...
            if request.method == "POST":
                ...
                if request.form["formType"] == "filter":
                    ...
                    dbcursor.execute(
                        ...
                        ORDER BY hotel, price;'.format(
                            srch=search,
                            start=start,
                            end=end,
                            cur=currencies[session["currency"]],
                        ),
                        (
                            datetime.strptime(request.form["from"], "%Y-%m-%d").month,
                            int(people) - 1,
                            people,
                            minprice,
                            maxprice,
                    ...
                    for prod in rows:
                        if (prod[0], prod[4]) in hotels:
                            hotels[(prod[0], prod[4])][prod[1]] = [prod[2], prod[3]]
                        else:
                            hotels[(prod[0], prod[4])] = {prod[1]: [prod[2], prod[3]]}
                    dbcursor.execute(
                        ...
                        ORDER BY hotel;'.format(
                            srch=search, start=start, end=end
                        ),
                        (
                            datetime.strptime(request.form["from"], "%Y-%m-%d").month,
                            minprice,
                    ...
                    return render_template(
                        ...
                        admin="logged_in" in session
                        and "@hhotels.co.uk" in session["email"],
            ...
            dbcursor.execute(
                ...
                WHERE re.Cancelled=false AND h.HotelLocation=re.HotelLocation AND r.RoomType=re.RoomType AND \
                ((re.StartDate BETWEEN "{start}" AND "{end}") OR ("{start}" BETWEEN re.StartDate AND re.EndDate))) as c \
                FROM `HOTELS` h CROSS JOIN `ROOMS` r HAVING c<m ORDER BY hotel, price;'.format(
                    start=start, end=end
                ),
                (months,),
            )
            ...
            for prod in rows:
                if (prod[0], prod[4]) in hotels:
                    hotels[(prod[0], prod[4])][prod[1]] = [prod[2], prod[3], prod[4]]
                else:
                    hotels[(prod[0], prod[4])] = {prod[1]: [prod[2], prod[3], prod[4]]}
            dbcursor.execute(
                ...
                WHERE re.Cancelled=false AND h.HotelLocation=re.HotelLocation AND \
                ((re.StartDate BETWEEN "{start}" AND "{end}") OR ("{start}" BETWEEN re.StartDate AND re.EndDate))) as c \
                FROM `HOTELS` h HAVING c>=m ORDER BY hotel;'.format(
                    start=start, end=end
                ),
            )
            rows = dbcursor.fetchall()
            ...
            return render_template(
                ...
                loggedin="logged_in" in session,
                currencies=currencies,
                admin="logged_in" in session and "@hhotels.co.uk" in session["email"],
                currentCurrency=session["currency"],
                fullybooked=rows,
                intervals=intervals,
                fromdate=start,
    ...
    return render_template(
        "base.html",
        error="There was an error connecting to database",
    )
    ...
    if conn != None:  # Checking if connection is None
        if conn.is_connected():
            ...
            if request.method == "POST":
                ...
                dbcursor.execute(
                    "SELECT IF(%s BETWEEN 4 AND 9, PricePeak, PriceOffPeak) FROM HOTELS WHERE HotelLocation = %s;",
                    (
                        datetime.strptime(startDate, "%Y-%m-%d").month,
                        hotel,
                    ),
                )
                hotelPrice = dbcursor.fetchone()[0]
                dbcursor.execute(
                    "SELECT PricePercentage, ExtraPriceForGuest FROM ROOMS WHERE RoomType = %s;",
                    (roomType,),
                )
                ...
                price = period * (
                    ...
                    * (
                        1
                        + float(typePrice[0])
                        + (float(typePrice[1]) * (int(people) - 1))
                    )
                )
                dbcursor.execute(
                    "SELECT UserFName, UserLName FROM USERS WHERE UserEmail = %s;",
                    (session["email"],),
                )
                user = dbcursor.fetchone()
                dbcursor.execute(
                    "SELECT Discount FROM DISCOUNTS WHERE %s BETWEEN LowerBound AND UpperBound;",
                    (discountDays,),
                )
    ...
    if conn != None:  # Checking if connection is None
        if conn.is_connected():
            ...
            if "logged_in" in session:
                ...
                dbcursor.execute("SELECT * FROM USERS WHERE UserEmail = %s;", (email,))
                x = dbcursor.fetchone()
                if dbcursor.rowcount > 0:  # this means there is a user with same name
                    ...
                    return render_template(
                        ...
                        and "@hhotels.co.uk" in session["email"],
                        hotels=hotelpics,
                        message="Registration was successful, please log in with your new account",
                    )
            else:
                return render_template(
                    "register_page.html",
        ...
        if conn.is_connected():
            ...
            if request.method == "POST":
                if request.form["formType"] == "loginForm":
                    ...
                    dbcursor.execute(
                        "INSERT INTO RESERVATIONS (UserEmail, HotelLocation, RoomType, StartDate, EndDate, TotalPrice, Currency, NumberOfPeople, DateOfBooking, Cancelled ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, false);",
                        (
                            session["email"],
                            hotel,
                            roomType,
                    ...
                    conn.commit()
                    dbcursor.execute("SELECT MAX(reservationId) FROM RESERVATIONS;")
                    ...
                    error = (
                        "<a href='/download_receipt/%s'><button>download receipt</button></a>"
                        % rid
                    )
                elif request.form["formType"] in [
                    "changePassword",
                    "adminchangePassword",
                ]:
                    ...
                    dbcursor.execute(
                        "SELECT PasswordHash FROM USERS WHERE UserEmail = %s;",
                        (session["email"],),
                    )
                    data = dbcursor.fetchone()
                    if sha256_crypt.verify(currentPassword, str(data[0])):
                        ...
                        dbcursor.execute(
                            "UPDATE USERS SET PasswordHash = %s WHERE UserEmail = %s;",
                            (
                                newPassword,
                                email,
                            ),
                    ...
                    if sha256_crypt.verify(password, str(data[0])):
                        ...
                        dbcursor.execute(
                            "UPDATE USERS SET UserEmail = %s, UserFName = %s, UserLName = %s, UserPhone = %s, UserAddress = %s WHERE UserEmail = %s;",
                            (
                                email,
                                fname,
                                lname,
                    ...
                    if "@hhotels.co.uk" in session["email"]:
                        ...
                        data = dbcursor.fetchone()
                        if not sha256_crypt.verify(
                            request.form["currentpassword"], str(data[0])
                        ):
                            error = "Booking cancellation was NOT successful, incorrect password"
                        else:
                            dbcursor.execute(
                                "UPDATE RESERVATIONS SET Cancelled=true WHERE ReservationId=%s AND UserEmail=%s;",
                                (
                                    request.form["resid"],
                                    request.form["email"],
                                ),
                        ...
                        dbcursor.execute(
                            "UPDATE RESERVATIONS SET Cancelled=true WHERE ReservationId=%s;",
                            (request.form["resid"],),
                        )
                        conn.commit()
                        message = "Booking cancellation was successful"
                    ...
                    if sha256_crypt.verify(request.form["password"], str(data[0])):
                        if "admin" not in request.form["formType"]:
                            ...
                            email = request.form["email"]
                        if (
                            "admin" in request.form["formType"]
                            and email == session["email"]
                        ):
                            error = "Account deletion was NOT successful, have to delete account from delete account"
                        else:
                            dbcursor.execute(
                                "SELECT COUNT(*) FROM RESERVATIONS WHERE UserEmail=%s AND StartDate>%s AND Cancelled=false",
                                (
                                    email,
                                    date.today(),
                                ),
                            ...
                            if dbcursor.fetchone()[0]:
                                ...
                                dbcursor.execute(
                                    "DELETE FROM USERS WHERE UserEmail=%s;",
                                    (email,),
                                )
                                conn.commit()
                                if "admin" not in request.form["formType"]:
                        ...
                        error = (
                            "Account deletion was NOT successful, incorrect password"
                        )
                    ...
                    if dbcursor.rowcount > 1:  # this mean no user exists
                        error = (
                            "Adding a new hotel was NOT successful, hotel already exist"
                        )
                    else:
                        pic.save(
                            os.path.join(
                                app.root_path, "static", "rooms pics", "%s.jpg" % name
                            )
                        )
                        dbcursor.execute(
                            "INSERT INTO HOTELS VALUES (%s, %s, %s, %s, %s);",
                            (
                                name,
                                rooms,
                                peak,
                    ...
                    if sha256_crypt.verify(request.form["password"], str(data[0])):
                        dbcursor.execute(
                            "DELETE FROM HOTELS WHERE HotelLocation=%s;",
                            (hotelid,),
                        )
                        conn.commit()
                        try:
                        ...
                        error = (
                            "Deleting a hotel was NOT successful, incorrect password"
                        )
                    ...
                    if curid not in currencies:
                        dbcursor.execute(
                            "INSERT INTO CURRENCY_CONV VALUES (%s,%s/100);",
                            (
                                curid,
                                curval,
                            ),
                    ...
                    description = request.form["Description"]
                    dbcursor.execute(
                        "UPDATE HOTELS SET HotelLocation = %s, PricePeak = %s, PriceOffPeak = %s, Description = %s WHERE HotelLocation = %s;",
                        (
                            hotelLocation,
                            pricePeak,
                            priceOffPeak,
                    ...
                    try:
                        ...
                        os.rename(
                            os.path.join(
                                app.root_path, "static", "rooms pics", "%s.jpg" % hotel
                            ),
                            os.path.join(
                                app.root_path,
                                "static",
            ...
            if "@hhotels.co.uk" not in session["email"]:
                dbcursor.execute(
                    ...
                    FROM RESERVATIONS rs WHERE UserEmail=%s",
                    (
                        date.today(),
                        session["email"],
                    ),
                ...
                dbcursor.execute(
                    ...
                    (SELECT COUNT(r.HotelLocation) FROM `RESERVATIONS` r \
                        WHERE h.HotelLocation=r.HotelLocation  AND r.EndDate>%s AND r.Cancelled=false)=0;",
                    (date.today(),),
                )
                hoteltodel = dbcursor.fetchall()
                dbcursor.execute(
                    'SELECT h.hotelLocation, (SELECT COUNT(*) FROM `RESERVATIONS` r WHERE h.HotelLocation=r.HotelLocation AND Cancelled=false AND StartDate BETWEEN "{sdate}" AND "{ndate}") as res,\
                                    ...
                                    '.format(
                        sdate=date.today() + timedelta(days=-30), ndate=date.today()
                    )
                )
...
def download(rid):
    return send_file(
        os.path.join(app.root_path, "receipts", "booking%s.txt" % rid),
        as_attachment=True,
    )
